...
public class TheSubChat extends Application {

    Stage myStage;
    private ChatClient chatClientObj;
    static VBox chatVBox;
    static HBox mainLayoutBox;
    static Scene chatScene;
    public static VBox currentChatBox; // Reference to the currently displayed chatBox
    int counter =0;
    ...
    public void start(Stage myStage) {
        chatScene = new Scene(initialize(myStage));
        myStage.setScene(chatScene);
        myStage.setTitle("TheSubscription");
        myStage.setHeight(650);
    ...
    public ScrollPane initialize(Stage myStage){
        // Initialize the main layout container
        mainLayoutBox = new HBox();
        ...
        leftVBox.getChildren().addAll(chooseBox,new TheSubChat().createSmallPools());
        // Create and add the initial chatVBox
        Label chatLabel = new Label("Hello from Java");
        chatVBox = new VBox(chatLabel);
        chatVBox.setPrefSize(900, 650);

        // Add components to main layout
        mainLayoutBox.getChildren().addAll(leftVBox, chatVBox);
        mainLayoutBox.setSpacing(10);
        //chatScene=new Scene(mainLayoutBox, 1300, 650);
        // primaryStage.setScene(chatScene);
        // primaryStage.show();
        hostButton.setOnAction(e->{
            counter=1;
            System.out.println("Clicked Host Chat BTn");
            System.out.println(counter);
        });
        memberButton.setOnAction(e->{
            counter=0;
            System.out.println("Clicked Member Chat BTn");
            System.out.println(counter);
        });
        ScrollPane chatScrollPane = new ScrollPane(mainLayoutBox);
        return chatScrollPane;
       
    }

    public  ScrollPane createSmallPools() {
        VBox poolVBox = new VBox(12);
        poolVBox.setMaxWidth(300);

        for (int i = 0; i < 20; i++) {
            Image poolImg = new Image("file:E:/TheSubscription(Prototype1)/thesubscription/src/main/resources/images/aws.png");
            Circle poolImgCircle = createPoolImage(poolImg);
            Label poolName = createPoolLabel("Ranjeet Pool");
            HBox poolHBox = createPoolHBox(poolImgCircle, poolName);
            Button poolButton = createButton(poolHBox);
            poolVBox.getChildren().add(poolButton);
            poolButton.setOnAction(e -> {
                if(counter==1){
                        ChatBox chatObj = new ChatBox(poolImgCircle, poolName);
                        VBox newChatBox = chatObj.createChatBox();

                        // Remove the existing chatVBox if any
                        if (chatVBox != null) {
                            mainLayoutBox.getChildren().remove(chatVBox);
                        }

                        // Remove the existing chatBox if any
                        if (currentChatBox != null) {
                            mainLayoutBox.getChildren().remove(currentChatBox);
                        }

                        // Add the new chatBox
                        mainLayoutBox.getChildren().add(newChatBox);
                        currentChatBox = newChatBox; // Update the reference to the new chatBox
                } else if(counter==0){
                System.out.println("Clicked HOST CHat Pool");
                HostChatBox hostChatObj = new HostChatBox(poolImgCircle, poolName);
                VBox newChatBox = hostChatObj.createMemberBox(4);
                // VBox newChatBox = hostChatObj.createHostChatBox();
        ...
        ScrollPane poolScrollPane = new ScrollPane(poolVBox);
    ...
    private Circle createPoolImage(Image poolImage) {
        Circle img = new Circle(30);
        img.setFill(new ImagePattern(poolImage));
        return img;
    }

    private Label createPoolLabel(String poolName) {
    ...
    private HBox createPoolHBox(Circle img, Label poolName) {
        ...
        HBox poolBox = new HBox(img, poolName);
        poolBox.setPrefSize(250, 100);
        poolBox.setAlignment(Pos.CENTER_LEFT);
        poolBox.setMargin(img, new Insets(0, 20, 0, 10));
        poolBox.setStyle("-fx-border-color: black; -fx-border-width: 2px; -fx-padding: 5px; -fx-background-color: white;");
    ...
    private Button createButton(HBox hbox) {
...
class ChatBox {
    ...
    VBox createChatBox() {
        System.out.println("Member Chat opened");
...
class HostChatBox {
    ...
    public VBox createHostChatBox() {
        System.out.println("HostChat Box called");
        Button reportButton = createReportButton();
        HBox topHBox = createTopBox(imgIcon, poolName, reportButton);
        ...
        chatBox.getChildren().addAll(topHBox, mainTextChatBox,msgBox);
    ...
    public VBox createMemberBox(int count) {
        VBox memberHolder = new VBox(20); // Add spacing between members
        ...
        for (int i = 0; i < count; i++) {
            ...
            VBox memberVBox = createEachMemberVBox(memberIcon, memberName);
            memberHolder.getChildren().add(memberVBox);
            Button eachMemberBtn = new Button();
            eachMemberBtn.setGraphic(memberVBox);
            eachMemberBtn.setOnAction(e->{
                VBox newChatBox = new HostChatBox(memberIcon,memberName).createHostChatBox();
                TheSubChat.mainLayoutBox.getChildren().remove(TheSubChat.currentChatBox);
                TheSubChat.mainLayoutBox.getChildren().add(newChatBox);
                TheSubChat.currentChatBox = newChatBox;
            });
        }
        return memberHolder;
    }

    private VBox createEachMemberVBox(Circle imgIcon, Label memberName) {
